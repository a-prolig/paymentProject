...
public class ClientProductDao {
    public List<ClientProduct> getAll() {
        ...
        try (Connection con = getDBConnection();
            ...
            while (rs.next() ) {
                ...
                clientProduct.setBalance(rs.getBigDecimal("balance"));
        ...
        try (Connection con = getDBConnection();
            ...
            clientProducts.forEach(product -> {
                try {
                    ...
                    pst.setBigDecimal(3, product.balance);
    ...
    public ClientProduct update(ClientProduct updateProduct) {
        String SQL_QUERY = "update clientproducts set balance = ? where id = ?";
        ClientProduct clientProduct = new ClientProduct();
        try (Connection con = getDBConnection();
            PreparedStatement pst = con.prepareStatement(SQL_QUERY)) {
            pst.setBigDecimal(1, updateProduct.getBalance());
            pst.setInt(2, updateProduct.getId());
            try {
                pst.executeUpdate();
                System.out.println("Database has been update");
            } catch (SQLException e) {
            ...
            clientProduct = getProduct(updateProduct.getId());
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return clientProduct;
